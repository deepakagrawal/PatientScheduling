...
import sys,getopt
import sys
sys.path.append("/opt/aci/sw/gams/24.8.5/apifiles/Python/api")
import pandas as pd
from model_code2 import generateDailyProb
import numpy as np
import itertools
import tempfile
...
def run_main(iter, data=None):
    ...
    return [profit[data.day0:data.Tmax].sum() / (data.Tmax - data.day0), patleft[data.day0:data.Tmax].sum()/(data.Tmax - data.day0)]
...
def calc_lambda(data,doubly):
    ...
    for i in range(data.lprob):
        data.l[0,:,i] = np.multiply(temp_demand,(1-(temp_new+temp_returning))).sum(axis=0) + \
                        np.multiply([1 if x>0 else 0 for x in np.multiply(temp_demand,(1-(temp_new+temp_returning))).sum(axis=0)],
                                    ((data.lprob-1.0)/2-i)*(np.multiply(temp_demand,(1-(temp_new+temp_returning))).sum(axis=0))*0.25)
        data.l[1,:,i] = np.multiply(temp_demand,temp_new).sum(axis=0) +\
                        np.multiply([1 if x > 0 else 0 for x in np.multiply(temp_demand,temp_new).sum(axis=0)],
                                    ((data.lprob - 1.0) / 2 - i) * (np.multiply(temp_demand, (1 - (temp_new + temp_returning))).sum(axis=0)) * 0.25)
        data.l[2:,:,i]= np.multiply(temp_demand,temp_returning) + \
                        np.multiply(np.int64(np.multiply(temp_demand,temp_returning)>0),
                                    ((data.lprob-1.0)/2-i)*(np.multiply(temp_demand,(1-(temp_new+temp_returning))).sum(axis=0))*0.25)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        Imax, processes, start, end, theta, dchoice_a, dchoice_b, loc_a, loc_b, policy, joint, doubly, nslots\
            = 1, 1, 5, 8, [1.25], 0, 2, 1, 2, ["SP"], [0], [0], 1
    else:
        myopts, args = getopt.getopt(sys.argv[1:], 'i:p:s:e:t:c:l:o:j:d:n:')
        for o,a in myopts:
            if o == "-i":
                Imax = int(a)
            elif o=="-p":
                            ...
                            for data.dynamic in policy:
                                for loc_dep in range(loc_a,loc_b):
                                    ...
                                    if data.dynamic == "DP":  # Dynamic
                                        ...
                                        # results = np.array(pool.map(functools.partial(run_main, data=data), range(Imax)))
                                    ...
                                    if results is not None:
                                        out = pd.DataFrame(np.array(
                                            [np.repeat(data.dd, Imax), np.repeat(data.theta, Imax), np.repeat(ch, Imax),
                                             np.repeat(data.dynamic, Imax),np.repeat(loc_dep, Imax),np.repeat(data.joint, Imax),np.repeat(dob,Imax), results[:, 0]]).T)
