...
*cb.l(kj,lj,i)$(ord(i)<= 1) = csign(kj,lj)

scalar UB /INF/;
scalar LB /-INF/;
set iter /iter0*iter10/;
set problem /rminlp, minlp/;
parameter gbdlog(iter, problem, *) 'algorithm log';
parameter cb_best(kj,lj,i) 'record best solution';
parameter x_best(dj,kj,tj,cj,lj) 'record best solution';
parameter u_best(cj,lj) 'record best solution';
...
loop(iter$(not done),
    ...
    u.up(cj,lj) = 1;
    u.lo(cj,lj) = 1 - sign(sum((dj,kj,tj),sign(v(dj,kj,tj,cj,lj))));
    x.lo(dj,kj,tj,cj,lj) = 0;

    If (Policy eq 0,
            Solve SP_loc_dep using rminlp maximizing obj;
        Elseif Policy eq 2,
            Solve DP_loc_dep using rminlp maximizing obj;
*           abort$(DP_loc_dep.solvestat <>1) "NLP Solver did not return OK";
*           abort$(DP_loc_dep.modelstat = 3 or DP_loc_dep.modelstat >= 6) "NLP Solver did not return OK";
           gbdlog(iter, 'rminlp', 'solvestat') = DP_loc_dep.solvestat;
           gbdlog(iter, 'rminlp', 'modelstat') = DP_loc_dep.modelstat;

           gbdlog(iter, 'rminlp', 'obj') = obj.l;
    );


    if (obj.l <UB, 
                UB = obj.l;
                gbdlog(iter, 'rminlp', 'better') = 1;
                obj.up = UB;
                obj.lo = LB;);
               gbdlog(iter, 'rminlp', 'UB') = UB;
           gbdlog(iter, 'rminlp', 'LB') = LB;

    cb.l(kj,lj,i) = floor(cb.l(kj,lj,i));
    
    x.fx(dj,kj,tj,cj,lj) = x.l(dj,kj,tj,cj,lj);
    u.fx(cj,lj) = u.l(cj,lj);

    If (Policy eq 0,
            Solve SP_loc_dep using minlp maximizing obj;
        Elseif Policy eq 2,
            Solve DP_loc_dep using minlp maximizing obj;
*           abort$(DP_loc_dep.solvestat <>1) "NLP Solver did not return OK";
*           abort$(DP_loc_dep.modelstat = 3 or DP_loc_dep.modelstat >= 6) "NLP Solver did not return OK";
           gbdlog(iter, 'minlp', 'solvestat') = DP_loc_dep.solvestat;
           gbdlog(iter, 'minlp', 'modelstat') = DP_loc_dep.modelstat;
           gbdlog(iter, 'minlp', 'obj') = obj.l;

    );  
    if (obj.l >LB, 
                LB = obj.l;
                gbdlog(iter, 'minlp', 'better') = 1;
                obj.lo = LB;
                obj.up = UB;);
               gbdlog(iter, 'minlp', 'UB') = UB;
           gbdlog(iter, 'minlp', 'LB') = LB;
    cb.fx(kj,lj,i) = cb.l(kj,lj,i);
    if(UB-LB<=0.01,
        done = 1);
...
def generateDailyProb(data, dynamic=None, cumulative=None, schedule=None, i=None, loc_dep=1, keepfiles=True):
    optim = JointOptimizer(keepfiles=keepfiles) if data.joint is 1 else SingleOptimizer(
        keepfiles=keepfiles)
