...
$if not set gdxincname $abort 'no include file name for data file provided'
$gdxin %gdxincname%
$load dj,kj,tj,nj,dd,theta,CAP,v,r,s,C,csign,L,prob, Policy,y,z, loc_dep
$gdxin
...
Variables
x(dj,kj,tj)  scheduling probability
u        leaving probability
obj               total profit
alphaS(kj,nj)    
P1S(kj,nj)
P2S(kj,nj)
...
total_profit_SP        Objective value SP
total_profit_DP        Objective value SP
eq2_loc_dep_1             Sum of probability 1 for flexible class when loc_dep = 1
eq2_1_loc_dep_1           Sum of probability 1 for flexible class when loc_dep = 1
eq2_loc_dep_0             Sum of probability 1 for flexible class when loc_dep = 0
eq2_1_loc_dep_0           Sum of probability 1 for flexible class when loc_dep = 0
eq4(dj,kj,tj) choice constraint
alphaEqS(kj,nj)
P1eqS(kj,nj)
P2eqS(kj,nj)
P3eqS(kj,nj)
...
total_profit_SP         .. obj =e= sum((dj,kj,tj,nj), prob(nj)*L(nj)*s(dj)*x(dj,kj,tj)) - 
                                   theta*sum((kj,nj),prob(nj)*(alphaS(kj,nj)*P1S(kj,nj) - C(kj)*P2S(kj,nj) + C(kj)*P3S(kj,nj) - alphaS(kj,nj)*P4S(kj,nj)));
total_profit_DP         .. obj =e= sum((dj,kj), sum((di,dx)$((ord(dx) eq ord(di) + ord(dj)-1) and (ord(di) ge 1 and ord(di) le dd+1 - ord(dj))), sum(tj,y(kj,tj,di,dx))*st(dx,di))+
                                   sum(nj,L(nj)*s(dj)*sum(tj,x(dj,kj,tj))*prob(nj)) + sum((nj,di)$((ord(di) ge 0) and (ord(di) le ord(dj)-1)),L(nj)*s(di)*sum(tj,z(di,kj,tj))*prob(nj)))-
                                   theta*sum((dj,kj,nj),prob(nj)*(alphaD(dj,kj,nj))*P1D(dj,kj,nj) - C(kj)*P2D(dj,kj,nj) + C(kj)*P3D(dj,kj,nj) - alphaD(dj,kj,nj)*P4D(dj,kj,nj));
eq2_loc_dep_1      .. u + sum((kj,dj,tj),x(dj,kj,tj)) =e= 1;
eq2_1_loc_dep_1    .. u + sum((kj,dj,tj),csign(kj)*x(dj,kj,tj)) =e= 1;
eq2_loc_dep_0      .. u + sum((kj,dj,tj),csign(kj)*x(dj,kj,tj)) =e= 1;
eq2_1_loc_dep_0    .. u + sum((kj,dj,tj),x(dj,kj,tj)) =e= 1;
eq4(dj,kj,tj)      .. x(dj,kj,tj)-v(dj,kj,tj)*u =l= 0;
*********** Static
alphaEqS(kj,nj)   .. alphaS(kj,nj) =e= sum((dj,tj), L(nj)*r(dj)*x(dj,kj,tj));
P1eqS(kj,nj)      .. P1S(kj,nj) =e= 1 - exp(-alphaS(kj,nj))*(sum(i$(ord(i)<=floor(C(kj))), alphaS(kj,lj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P2eqS(kj,nj)      .. P2S(kj,nj) =e= 1 - exp(-alphaS(kj,nj))*(sum(i$(ord(i)<=floor(C(kj))+1), alphaS(kj,lj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P3eqS(kj,nj)      .. P3S(kj,nj) =e= exp(-alphaS(kj,nj))*(sum(i$(ord(i)<=ceil(C(kj))-1), alphaS(kj,lj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P4eqS(kj,nj)      .. P4S(kj,nj) =e= exp(-alphaS(kj,nj))*(sum(i$(ord(i)<=ceil(C(kj))-2), alphaS(kj,lj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));

*********** Dynamic
alphaEqD(dj,kj,nj)   .. alphaD(dj,kj,nj) =e= sum((di,dx)$((ord(dx) eq ord(di)+ord(dj)-1) and (ord(di) ge 1) and (ord(di) le dd +1 - ord(dj))),sum(tj,y(kj,tj,di,dx))*rt(dx,di)) + 
L(nj)*r(dj)*sum(tj,x(dj,kj,tj)) + sum((di)$((ord(di) ge 1) and (ord(di) le ord(dj)-1)),L(nj)*r(di)*sum(tj,z(di,kj,tj)));
P1eqD(dj,kj,nj)      .. P1D(dj,kj,nj) =e= 1 - exp(-alphaD(dj,kj,nj))*(sum(i$(ord(i)<=floor(C(kj))), alphaD(dj,kj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P2eqD(dj,kj,nj)      .. P2D(dj,kj,nj) =e= 1 - exp(-alphaD(dj,kj,nj))*(sum(i$(ord(i)<=floor(C(kj))+1), alphaD(dj,kj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P3eqD(dj,kj,nj)      .. P3D(dj,kj,nj) =e= exp(-alphaD(dj,kj,nj))*(sum(i$(ord(i)<=ceil(C(kj))-1), alphaD(dj,kj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));
P4eqD(dj,kj,nj)      .. P4D(dj,kj,nj) =e= exp(-alphaD(dj,kj,nj))*(sum(i$(ord(i)<=ceil(C(kj))-2), alphaD(dj,kj,nj)**ord(i)/prod(fi$(ord(fi) le ord(i)),ord(fi)) ));


MODEL SP_loc_dep_1 /eq2_loc_dep_1,eq2_1_loc_dep_1,eq4,alphaEqS,P1eqS,P2eqS,P3eqS,P4eqS, total_profit_SP/;
MODEL DP_loc_dep_1 /eq2_loc_dep_1,eq2_1_loc_dep_1,eq4,alphaEqD,P1eqD,P2eqD,P3eqD,P4eqD, total_profit_DP/;

MODEL SP_loc_dep_0 /eq2_loc_dep_0,eq2_1_loc_dep_0,eq4,alphaEqS,P1eqS,P2eqS,P3eqS,P4eqS, total_profit_SP/;
MODEL DP_loc_dep_0 /eq2_loc_dep_0,eq2_1_loc_dep_0,eq4,alphaEqD,P1eqD,P2eqD,P3eqD,P4eqD, total_profit_DP/;
...
*Display x.l, x.m,obj.l,cj ;  
    ...
    def solve(self, data, dynamic,schedule, loc_dep_inp):
        ...
        v = db.add_parameter_dc("v", [dj, kj, tj], "Choice")
        r = db.add_parameter_dc('r', [dj], "retaining prob")
        s = db.add_parameter_dc('s', [dj], "show up prob")
        L = db.add_parameter_dc('L', [nj], "arrival rate")
        C = db.add_parameter_dc('C', [kj], "Capacity Constraint Parameter")
        csign = db.add_parameter_dc('csign', [kj], "Capacity Constraint Parameter")
        prob = db.add_parameter_dc('prob', [nj], "doubly stochastic prob")
        y = db.add_parameter_dc('y', [kj, tj, dj, dj], 'Current Schedule')
        z = db.add_parameter_dc('z', [dj, kj, tj], 'Static Prob')

        for key, val in zip(range(1,data.dd+1), data.r):
            r.add_record(str(key)).value = val
            s.add_record(str(key)).value = val
        for key, val in np.ndenumerate(data.v[:,:,:,1,0]):
            v.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(data.l[1,0,:]):
            L.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(data.C[:,0]):
            C.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(data.csign[:,0]):
            csign.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(data.probl):
            prob.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(schedule[:,1,0,:,:,:]):
            y.add_record([str(i+1) for i in key]).value = val
        for key, val in np.ndenumerate(data.Prob[:,:,:,1,0]):
            z.add_record([str(i+1) for i in key]).value = val
