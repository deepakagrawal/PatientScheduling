...
import sys
import pandas as pd
from model_code1_jug import generateDailyProb
import numpy as np
import itertools
import tempfile
...
if __name__ == "__main__":
    ...
    tmp_demand = np.tile(demand, (horizon[-1], 1, 1))
    tmp_demand[tmp_demand != 0] = 1
    demand[demand != 0] = 5
    varray1 = calcFun(data.dk,data.dl,horizon[-1],demand)
                            ...
                            for data.dynamic in policy:
                                for loc_dep in range(loc_a,loc_b):
                                    ...
                                    if data.dynamic == "DP":  # Dynamic
                                        ...
                                        rank = comm.Get_rank()
                                        size = comm.Get_size()
                                        results = run_main(iter =rank, data = data)
                                    ...
                                    if results is not None:
                                        if rank==0:
                                            ...
                                            dat_C.append(pd.concat([result[2] for result in results]))
